...
LT(1, KC_ESC), KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,          KC_ENT,  KC_VOLU,  \
  ...
  [1] = KEYMAP_ANSI(
      ...
      _______, KC_MPRV, KC_MPLY, KC_MNXT, _______, _______, _______, _______, _______, M(0),    _______, _______, _______, _______, KC_MPRV,  \
      _______, _______, KC_VOLD, KC_VOLU, KC_MUTE, _______, KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT, M(1),    _______,          _______, KC_MUTE,  \
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt) {
  if (record->event.pressed) {
    switch(id) {
      case 0:
        return MACRO(D(LCTL), T(A), U(LCTL), T(O));
      case 1:
        return MACRO(D(LCTL), T(A), U(LCTL), T(SCLN));
    }
  }
  return MACRO_NONE;
};
